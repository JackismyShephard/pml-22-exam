...
# ╔═╡ 747ba706-e5c3-4796-bcba-d808d9389e5f
begin
	using Random
	using Distributions
	using LinearAlgebra
...
# ╔═╡ 833804da-ca92-4673-a67a-36fe29550594
begin
	using DataFrames
	table = DataFrame(
					a=[0,0,0,0,1,1,1,1], 
	...
	function test_conditional(a, b, c, table::DataFrame=table)
		...
		p_a_b_c = sum(filter(row -> row.a == a && row.b==b && row.c==c, table).p_abc)/p_c
		println("p(a=$a,b=$b|c=$c)=$p_a_b_c = $p_a_c*$p_b_c=p(a=$a|c=$c)p(b=$b|c=$c)")
...
# ╔═╡ 499aa7ec-6a38-11ed-1251-8b6a0014d174
md"""
# Exercise Set 1 | PML Fall 2022
Unless indicated otherwise the exercises refer to "Pattern Regognition and Machine Learning" by CM Bishop (Springer) 2006.
## Theory
### Basic Probability Theory
#### 1.10) 
Let $x,y$ be independent random variables, then the linear property of E is:
the expectation is $E[x+z] = \sum_x\sum_z p(x,z)(x+z)$
$=\sum_x\sum_z p(x)p(z)(x)+\sum_x\sum_z p(x)p(z)(z)$
$=\sum_x p(x)x\sum_zp(z)+\sum_zp(z)z\sum_xp(x)=E[x]\times 1 + E[z]\times 1$,

and variance as $Var[x+z] = E[(x+z)^2]-E[(x+z)]^2$
$= E[x^2+2xz+z^2] - (E[x]+E[z])^2$
$= E[x^2]+2E[x]E[z]+E[z^2]-E[x]^2-2E[x]E[z]-E[z]^2$
$= E[x^2]-E[x]^2 + E[z^2]-E[z]^2 =Var[x] + Var[z]$ .

#### 2.8)
Consider variables $x,y$ jointly distributed with $p(x,y)$, then:
$E_y[E_x[x|y]]=E_y[\sum_xp(x|y)x]=\sum_yp(y)\sum_xp(x|y)x=\sum_xp(x)x=E[x]$.

For the variance it is the case that:
$E_y[V_x[x|y]]=E_y[E_x[x^2|y]-E_x[x|y]^2]=E_y[E_x[x^2|y]]-E_y[E_x[x|y]^2]=E_x[x^2]-E_y[E_x[x|y]^2]$ and
$V_x[E_x[x|y]]=E_y[E_x[x|y]^2]-E_y[E_x[x|y]]^2$
therefore it follows that $V[x]=E_x[x^2]-E_y[E_x[x|y]^2]+E_y[E_x[x|y]^2]-E_y[E_x[x|y]]^2=E_x[x^2]-E_x[x]^2$.

#### 2.20)
A p.d. real valued matrix $\Sigma$ defined in the quadratic form as $a^T\Sigma a$ is positive for any real value of vector $a$, because (given symmetry and decomposition) and following matrix notation for (2.45) and (2.48) s.t. $U^Ta=y\neq 0$ then:
$a^T\Sigma a = a^T(U^T\Lambda U)a=y^T\Lambda y=\sum_i\lambda_i y_i^2 >0$.
Here matrix symmetry and real values allow us to use _spectral decomposition_.


Otherwise using (2.45) : $a^T\Sigma a=\sum_i\lambda_i u_i^T\Sigma\sum_j\lambda_j u_j=\sum_{ij}\lambda_i\lambda_ju_i\Sigma u_j$ and as orthonormal basis we can write $\Sigma u_j = \phi_j u_j$ for eigenvalues $\phi$, then
$\Rightarrow \sum_{ij}\lambda_i\lambda_j\phi_j u_iu_j=\sum_{i}\lambda_{i}^2\phi_{i}>0$.

See Matrix Cookbook (293) section 5.3.1 applicable to real symmetric matrices.
"""

# ╔═╡ 71d0b49b-6448-494b-9ff6-86bd8edfaa30
md"""
#### 8.3) 
Evalutating the joint distribution over three binary variables
"""

# ╔═╡ aeee2bc8-42f6-4160-ae5b-f9c053e88824
md"""
#### 8.4)
...
# ╔═╡ 7848e414-b3f6-4aac-94b0-2223a2f2d3fa
md"""
#### 8.9)
D-separation, we recall:

i) [A] ---> [X] ---> [B] ;  [A] <--- [X] ---> [B]    then A$\perp \!\!\! \perp$B|X,
if X is in the conditioning set.

ii) [A]-->[X]<--[B] A$\perp \!\!\! \perp$B if X not in conditioning set.


For the Markov Blanket see the definition from Bishop (Fig.8.26) and Lecture notes. 
The answer follows directly from the definition in these materials.
...
# ╔═╡ d1880845-916b-405a-aa7e-472dc800a700
begin
	p_b1=0.9
	p_b0=1-p_b1
	p_f1=0.9
...
# ╔═╡ 62421bc1-6d1a-4670-ba9d-05e2aeef9593
md"""
## Programming Exercise
"""

# ╔═╡ 170ed704-0c44-4b3c-8206-866359c4ec8f
md"""
###### Solve for D: $(@bind option CheckBox())
"""

# ╔═╡ e0cdf86e-dde6-4cc2-9ccf-d285beb3b052
begin
	N=2
...
# ╔═╡ 20751a7d-4d8e-4b91-8c1c-f11879ab36d7
md"""
#### A) 
...
# ╔═╡ 90da094a-218c-41f9-82a8-52a318cecae7
begin
	sample_size=20
	dim = 2
	μ = [0., 0.]
...
# ╔═╡ 6521cc0d-8f0d-4975-9ae1-e304db3ab104
md"""
#### B)
...
# ╔═╡ 9e94afde-b03e-4602-a839-a7ed7ebad540
begin
...
# ╔═╡ 3620d554-7961-4484-a5a4-79c5dd18269c
begin
# plot PDF
	...
	heatmap(values', clims=(0, 15), xaxis=nothing, yaxis=nothing)
end

# ╔═╡ bbc72d10-618b-48be-8c4b-c23a97aab12b
md"""
#### C) 
Compute Variance of posterior predictive
"""

# ╔═╡ dca7b40a-c345-424d-a24c-2b3e2cfd6c71
begin
	variance = sum((XX*Σ_θ).*XX, dims=2) .+ σ
	variance = reshape(variance, (size(θ_range)[1], size(θ_range)[1]))
	heatmap(variance', clims=(0., 0.4), xaxis=nothing, yaxis=nothing)
end
